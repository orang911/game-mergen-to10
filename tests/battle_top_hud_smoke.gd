...
func _run() -> void:
	...
	_check(_path_ends(currency_icon.texture, "ui/interfaces/battle/top_hud/icons/battle_currency_icon.png"), "currency should use the organized currency icon")
	...
	_check(_rect_matches(timer_icon, Vector2(519, 30), Vector2(37, 37)), "clock should match the updated reference coordinate")
	...
	_check(_rect_matches(pause_button, Vector2(30, 14), Vector2(84, 88)), "pause layout should match the supplied reference")
	...
	_check(_visual_size_matches(currency_frame, Vector2(153, 84)), "currency nine-patch should retain enough height for its padded source art")
	...
	_check(_path_ends(skill_disabled.texture, "ui/interfaces/battle/energy_hud/icons/skill_disabled_icon.png"), "inactive skill should use the organized icon")
