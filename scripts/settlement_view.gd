...
const V2 := "res://assets/UI/结算界面/拆分素材_v2/"
const V3 := "res://assets/UI/结算界面/拆分素材_v3_原图对齐/"
const FRAME := V3 + "分区框/"

const PANEL_TEXTURE := preload(V2 + "底板_结算空白面板.png")
const WIN_TITLE := preload(V2 + "标题_胜利.png")
const LOSE_TITLE := preload(V2 + "标题_失败.png")
const RECORD_TEXTURE := preload(V3 + "标记_新纪录.png")
const REWARD_PANEL := preload(V3 + "底板_本局奖励_空白.png")
const REWARD_CHEST := preload(V3 + "奖励_宝箱.png")
const REWARD_COIN := preload(V3 + "奖励_金币.png")
const REWARD_CRYSTAL := preload(V3 + "奖励_绿色晶体.png")

const TAB_STATS := preload(FRAME + "分页_本局战绩_激活.png")
const TAB_DECK := preload(FRAME + "分页_本局卡组_未激活.png")
const MAIN_HIGHEST := preload(FRAME + "主数据框_最高数字_蓝.png")
const MAIN_KILLS := preload(FRAME + "主数据框_击杀怪物_紫.png")
const SUMMARY_FRAME := preload(FRAME + "分区框_波次时长分数_三段.png")
const SECONDARY_LEFT := preload(FRAME + "次级数据框_左_空白.png")
const SECONDARY_RIGHT := preload(FRAME + "次级数据框_右_空白.png")
const DAMAGE_FRAME := preload(FRAME + "伤害构成_外框_空白.png")
const DAMAGE_ROW := preload(FRAME + "伤害构成_单行底框.png")

const ICON_HIGHEST := preload(V3 + "统计_最高数字10.png")
const ICON_KILLS := preload(V3 + "统计_击杀怪物.png")
const ICON_MERGES := preload(V3 + "统计_合成次数.png")
const ICON_MAX_MERGE := preload(V3 + "统计_最大单次合成.png")
const ICON_CASTLE := preload(V3 + "统计_城堡耐久.png")
const ICON_LEAKS := preload(V3 + "统计_漏怪.png")
const ICON_BOARD_DAMAGE := preload(V3 + "伤害_棋盘合成.png")
const ICON_CRYSTAL_DAMAGE := preload(V3 + "伤害_水晶塔.png")
const ICON_SKILL_DAMAGE := preload(V3 + "伤害_道具技能.png")
