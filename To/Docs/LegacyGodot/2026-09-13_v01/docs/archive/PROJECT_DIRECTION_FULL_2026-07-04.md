...
目标：把可替换、可拼装、可加光效的视觉部分拆成独立场景，让后期换图、调整层级、增加特效时不污染系统逻辑。

核心原则：

- System 负责规则，View 负责表现。
- 系统脚本只管理数据、流程、信号和计算。
- 可视化场景负责贴图、几何形状、动画、光效、挂点和层级。
- 第一阶段先拆关键战斗可视化场景，不一次性拆太多。
- 旧玩法保持稳定，视觉拆分跟随第一阶段战斗闭环逐步落地。

#### 第一批可视化场景

第一批建议拆 6 个核心场景，服务于怪物路径、城堡耐久、弹道和特效。

| 场景 | 职责 | 说明 |
| --- | --- | --- |
| `res://scenes/combat/battle_layer.tscn` | 战斗视觉总层 | 承载怪物路径、怪物层、弹道层、特效层、城堡耐久 UI、波次 UI。只负责摆放和承载，不计算伤害，不生成怪物。 |
| `res://scenes/combat/monster_path_view.tscn` | 回形路径视觉 | 包含 `Path2D`、半透明轨道线、入口标记、终点标记。后续可替换成发光轨道、石板路、能量通道等。 |
| `res://scenes/combat/monster_view.tscn` | 怪物表现 | 小/中/大怪共用，通过参数改变尺寸、颜色、形状。包含几何造型、受击闪烁、血条挂点、死亡特效挂点。 |
| `res://scenes/combat/castle_view.tscn` | 城堡耐久表现 | 显示城堡图标、耐久数字、受击震动、裂纹、闪光。耐久数值仍由 `CastleSystem` 管理。 |
| `res://scenes/combat/projectile_view.tscn` | 弹道表现 | 表现攻击从棋盘飞向怪物。后续支持直线、弧线、连锁、穿透、爆炸、追踪。 |
| `res://scenes/combat/effect_layer.tscn` | 特效承载层 | 专门承载合成光效、命中特效、死亡爆裂、飘字、属性状态特效。 |
...
- `BattleLayer` 放在主游戏 `Game` 层下。
- `MonsterPathView` 负责路径视觉和 `Path2D`。
- `MonsterLayer` 承载怪物实例或怪物视觉节点。
- `ProjectileLayer` 承载弹道节点，避免弹道和怪物层互相干扰。
- `EffectLayer` 承载飘字、爆裂、光效等短生命周期表现。
- `CastleView` 放在终点附近或顶部安全区域。
- `WaveView` 第一阶段可以先简单显示文字，后续可独立拆到 `ui/` 目录。

#### 场景与系统对应关系

| 系统 | 使用的可视化场景 | 关系 |
| --- | --- | --- |
| `CombatSystem` | `battle_layer.tscn` | 创建或引用战斗视觉总层。 |
| `PathSystem` | `monster_path_view.tscn` | 控制回形 `Path2D`，根据棋盘位置更新路径。 |
| `MonsterSystem` | `monster_view.tscn` | 生成怪物表现，并维护怪物列表和路径进度。 |
| `CastleSystem` | `castle_view.tscn` | 更新耐久显示，播放受击反馈。 |
| `ProjectileSystem` | `projectile_view.tscn` | 创建弹道表现，管理飞行和命中动画。 |
| `EffectSystem` | `effect_layer.tscn` | 播放合成、命中、死亡、属性、飘字等表现。 |
...
| 场景 | 职责 |
| --- | --- |
| `res://scenes/blocks/block_view.tscn` | 方块视觉，负责数字/字母、颜色、选中态、抖动、缩放。当前已新增为预览场景。 |
| `res://scenes/blocks/board_grid.tscn` | 棋盘格可视化，负责棋盘背景、格子线、坐标辅助显示。当前已新增为预览场景。 |
| `res://scenes/ui/game_hud.tscn` | 分数、最高分、城堡耐久、波次等统一 HUD。 |
| `res://scenes/ui/wave_view.tscn` | 波次显示。 |
| `res://scenes/ui/loading_view.tscn` | Web 分段加载界面。 |
| `res://scenes/effects/damage_text.tscn` | 飘伤害数字。 |
| `res://scenes/effects/merge_burst.tscn` | 合成瞬间光效。 |

#### 格子合成可视化场景检索记录

当前已检索到两个与格子合成相关的可视化场景。

| 场景 | 脚本 | 当前用途 | 接入状态 |
| --- | --- | --- | --- |
| `res://scenes/blocks/block_view.tscn` | `res://scripts/block_preview.gd` | 方块视觉预览。根节点为 `Control`，节点名 `BlockView`。脚本使用 `@tool`，支持导出 `level` 和 `selected`，会读取 `plate_01` 到 `plate_10` 的 down/up 贴图，并显示等级文字。 | 目前未在 `main_game.gd` 主流程中引用，暂时属于编辑器预览/视觉拆分占位。 |
| `res://scenes/blocks/board_grid.tscn` | `res://scripts/board_grid_preview.gd` | 棋盘格预览。根节点为 `Control`，节点名 `BoardGrid`。脚本使用 `@tool`，读取棋盘背景 `bg_02.png`，根据 `GameConfig.GRID_SIZE` 和 `GameConfig.BLOCK_SIZE` 绘制格子线，可选显示坐标标签。 | 目前未在 `main_game.gd` 主流程中引用，暂时属于编辑器预览/视觉拆分占位。 |

记录结论：

- 格子合成相关视觉场景已经开始拆分。
- 当前拆分主要是预览层和占位层，尚未替换运行时 `MergeBlock` 和 `board_layer` 的逻辑。
- `block_view.tscn` 当前只支持 1-10 的贴图和文字显示，字母阶段还需要后续扩展。
- `board_grid.tscn` 当前适合用于布局预览、格子坐标检查和后续棋盘视觉替换参考。
- 后续如果要正式接入，需要决定是让现有 `block.gd` 逐步吸收 `BlockView`，还是让 `MergeBlock` 逻辑和 `BlockView` 表现彻底拆开。
...
scenes/
  combat/
    battle_layer.tscn
    monster_path_view.tscn
    monster_view.tscn
    castle_view.tscn
    projectile_view.tscn
    effect_layer.tscn
  blocks/
    block_view.tscn
    board_grid.tscn
...
- 第一阶段优先拆 `battle_layer.tscn`、`monster_path_view.tscn`、`monster_view.tscn`、`castle_view.tscn`。
- `projectile_view.tscn` 可以先建立占位，第二阶段合成攻击接入时再完善。
- `effect_layer.tscn` 可以先作为空层存在，后续逐步加入光效和飘字。
- 可视化场景不能直接决定伤害、血量、波次和失败。
- 视觉节点可以发出动画完成信号，但最终游戏状态由系统脚本控制。
- 场景中预留挂点，例如 `HitAnchor`、`HpAnchor`、`EffectAnchor`、`ProjectileOrigin`。
- 可视化场景命名统一使用 `_view` 后缀，方便和系统脚本区分。

#### 后期替换与加光效要求

- 怪物造型替换时，只改 `monster_view.tscn` 或其子资源。
- 路径视觉替换时，只改 `monster_path_view.tscn`，不改 `PathSystem` 的路径规则。
- 城堡 UI 替换时，只改 `castle_view.tscn`，不改耐久计算。
- 攻击弹道升级时，在 `projectile_view.tscn` 和 `ProjectileSystem` 中扩展，不改合成系统。
- 合成光效、命中特效、死亡爆裂、属性特效统一放到 `EffectSystem` 和 `effect_layer.tscn`。
- Web 版本要注意特效数量和透明叠加，避免移动浏览器性能下降。
...
- `Path2D + PathFollow2D` 在 Godot 4.x 中是成熟方案。
- 回形路径只需要根据 `board_pos` 和 `board_size` 计算 5 个点。
- 怪物可以先用 `Polygon2D` 或 `ColorRect` 做抽象几何怪，不依赖额外美术素材。
- 波次使用配置表驱动，逻辑清晰。
- 当前文档中的接口设计，包括信号、字段、方法，已经可以直接翻译成代码。

执行建议：

- 第一阶段可以作为最先落地的战斗闭环。
- 每完成一个子系统就运行一次主场景，不等全部完成后再统一测试。
...
- 伤害计算和目标选择逻辑相对简单。
- 主要风险在于攻击动画与棋盘下落、补充流程之间的时序协调。
- 弹道表现和异步播放需要避免阻塞棋盘操作。

执行建议：

- 合成成功后只发攻击事件，仍由 `CombatSystem` 处理战斗。
- 第二阶段先做普通伤害和简单命中反馈，不急着加入完整属性效果。
...
- 怪物需要维护状态字段或状态列表，例如 `burn_time`、`freeze_time`、`poison_time`。
- 持续伤害需要在 `_process(delta)` 或统一状态更新中结算。
- 属性效果之间的交互需要仔细测试。

执行建议：

- 先分别实现火焰、冰冻、雷电、毒的最小效果。
- 再考虑叠加、刷新持续时间、免疫、优先级等复杂规则。
...
- UI、特效、道具系统会显著增加表现和调参工作量。
- 建议在战斗闭环、合成攻击、基础属性都稳定后再进入。

### 关键风险点

#### `main_game.gd` 职责过重

当前 `main_game.gd` 已承担过多职责。文档已规划 BoardSystem、MergeSystem、GameStateSystem 等拆分方案，但第一阶段仍需要在 `main_game.gd` 中接入战斗层，可能进一步增加复杂度。

应对策略：

- 第一阶段只做必要接入，不在 `main_game.gd` 中堆战斗细节。
- 战斗逻辑放入 `CombatSystem` 及其子系统。
- 后续再拆 BoardSystem / MergeSystem。

#### 竖屏空间紧张

当前 5x5 棋盘约 `660x660`，外圈怪物路径还需要额外边距、城堡 UI 和波次显示。在 `720x1280` 等竖屏尺寸下需要精确布局。

应对策略：

- 使用 `BattleLayer` 管理怪物路径。
- 怪物路径不能遮挡棋盘点击区域。
- 城堡耐久和波次显示尽量贴近路径终点或顶部空区。

#### 两种失败来源冲突

未来会同时存在“棋盘无可合并”和“城堡耐久归零”两个失败来源。当前代码中 `check_fail()` 会直接触发 `end_game(false)`，加入战斗后可能需要调整。

待讨论：

- 棋盘无解时是否仍然直接失败。
- 是否给玩家刷新、道具、复活或等待怪物压力继续推进的机会。
- 失败结算是否记录失败原因。

#### Godot Web 导出未验证

当前记录中提到命令行找不到 Godot，因此还没有完成 Godot 真实解析、启动和 Web 导出检查。

应对策略：

- 先用 Godot 编辑器打开主场景，确认当前空实现接入无报错。
- 再验证 Godot 4.x Web 导出模板是否可用。
- Web 导出验证通过后，再推进分段加载系统。

#### 字母阶段素材缺失

当前 1-10 有现成素材，A 及以上还没有明确素材方案。

应对策略：

- 第一版可以先继续使用程序化文字显示字母。
- 后续根据视觉风格补字母阶段素材。
- `BlockView` 应支持数字和字母两种显示方式。

### 建议优先执行顺序

1. 打开 Godot 编辑器运行主场景，确认当前空实现不报错。
2. 按第一阶段 10 步顺序逐个实现怪物路径、波次和城堡耐久。
3. 每完成一个子系统就运行一次，不攒到最后再测。
4. 第一阶段闭环稳定后，再进入第二阶段合成攻击接入。
5. 合成攻击稳定后，再做颜色属性和高级表现。

结论：

- 第一阶段技术可行性高，适合作为下一步实现目标。
- 最大风险不是单点技术，而是系统职责继续堆在 `main_game.gd` 中。
- 当前最稳妥路线是保持原玩法不变，把战斗系统作为独立层逐步接入。
...
| 2026-07-03 | 补充技术可行性分析 | 第一阶段怪物路径、波次、城堡耐久为低风险；第二阶段合成攻击和第三阶段颜色属性为中等风险；记录 main_game 职责过重、竖屏空间、失败来源冲突、Web 导出未验证和字母素材缺失等关键风险。 |
| 2026-07-03 | 补充可视化场景拆分方案 | 规划 `battle_layer`、`monster_path_view`、`monster_view`、`castle_view`、`projectile_view`、`effect_layer` 等可视化场景，方便后期拼图、替换素材和增加光效。 |
| 2026-07-03 | 记录格子合成可视化场景 | 检索到 `scenes/blocks/block_view.tscn` 和 `scenes/blocks/board_grid.tscn`，分别用于方块视觉预览和棋盘格预览，目前尚未接入主游戏运行流程。 |
...
- 2026-07-03：技术可行性分析补充：第一阶段低风险、工作量明确；第二阶段合成攻击接入和第三阶段颜色属性为中等风险；第四/五阶段 UI 反馈和道具作为远期规划。关键风险包括 `main_game.gd` 职责过重、竖屏空间紧张、棋盘无可合并与城堡耐久归零两个失败来源冲突、Godot Web 导出未验证、字母阶段素材缺失。建议先运行 Godot 主场景，再逐个实现第一阶段子系统。
- 2026-07-03：可视化场景拆分补充：为后期拼图、替换素材、增加光效，规划第一批视觉场景为 `battle_layer.tscn`、`monster_path_view.tscn`、`monster_view.tscn`、`castle_view.tscn`、`projectile_view.tscn`、`effect_layer.tscn`。原则是 System 负责规则，View 负责表现；当前仅记录规划，尚未创建这些场景文件。
- 2026-07-03：格子合成可视化场景检索记录：当前目录中已新增 `scenes/blocks/block_view.tscn` 和 `scenes/blocks/board_grid.tscn`。`block_view.tscn` 使用 `block_preview.gd` 做 1-10 方块贴图和选中态预览；`board_grid.tscn` 使用 `board_grid_preview.gd` 绘制棋盘背景、格子线和坐标标签。两者目前未被 `main_game.gd` 主流程引用，暂时属于编辑器预览/视觉拆分占位。
